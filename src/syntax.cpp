...
namespace Wintermute {
	namespace Linguistics {
		...
		const string Node::id () const { return m_lxdt->id(); }

		const string Node::locale() const { return m_lxdt->locale(); }

		const string Node::symbol() const { return m_lxdt->symbol (); }

		const Leximap* Node::flags() const { return m_lxdt->flags (); }
		...
		NodeVector FlatNode::expand(const Node& p_nod){
			...
			for (Leximap::const_iterator itr = l_map->begin (); itr != l_map->end (); ++itr){
				...
				Node l_nd(l_lxdt);
				l_vtr.push_back(l_nd);
			}

			//cout << "(linguistics) [FlatNode] Expanded symbol '" << p_nod.symbol () << "' to spread across its " << l_map->size() << " variations." << endl;
		...
		const FlatNode* Link::source() const { return &m_src; }

		const FlatNode* Link::destination() const { return &m_dst; }

		const string Link::locale() const { return m_lcl; }
		...
		const Link* Link::fromString(const string& p_data) {
			...
			const string node1 = *itr; itr++;
			const string node2 = *itr; itr++;
			const string flags = *itr; itr++;
			const string lcl = *itr;
